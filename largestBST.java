...
class largestBST {
	static class BinaryTree {
		...
		private Info largestBSTinBT(Node root) {
            ...
            Info leftans = largestBSTinBT(root.left);
			Info rightans = largestBSTinBT(root.right);
			Info BST_info = new Info();
			boolean isbal = leftans.max < root.data && root.data < rightans.min && leftans.isBST && rightans.isBST;
			BST_info.max = Math.max(root.data,leftans.max);
			BST_info.min = Math.min(root.data,rightans.min);
			if(!isbal){
				BST_info.ans = Math.max(leftans.ans, rightans.ans);
			}
			else{
				BST_info.size = 1 + leftans.ans + rightans.ans;
				BST_info.ans = BST_info.size;
			}
			BST_info.isBST = isbal;
			return BST_info;
		}
